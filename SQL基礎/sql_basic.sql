...
SELECT celebrities.name, countries.name, language FROM celebrities 
JOIN countries on celebrities.country_code = countries.code 
JOIN countrylanguages on celebrities.country_code = countrylanguages.country_code AND is_official = 'T';
...
GROUP BY country_code HAVING MAX(age) >= 50 AND MIN(age) <= 30;

-- 問31
-- 1991年生まれと、1981年生まれの有名人が何人いるか調べてください。ただし、日付関数は使用せず、UNION句を使用してください。
SELECT 1980 as '誕生年', count(birth) FROM celebrities WHERE LEFT(birth, 4) = 1980 UNION
SELECT 1981 as '誕生年', count(birth) FROM celebrities WHERE LEFT(birth, 4) = 1981;

-- 問32
-- 有名人の出身国の平均年齢を高い方から順に表示してください。ただし、FROM句はcountriesテーブルとしてください。
SELECT countries.name as '国名', AVG(celebrities.age) as '平均年齢' FROM countries JOIN celebrities on countries.code = celebrities.country_code GROUP BY countries.code ORDER BY AVG(celebrities.age) DESC;
